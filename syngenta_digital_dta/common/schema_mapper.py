...
def map_to_schema(data, schema_key):
    model_data = {}
    model_schema = _get_model_schema(schema_key)
...
def _get_model_schema(schema_key):
    with open('openapi.yml') as openapi:
...
def _populate_model_data(properties, data, model_data):
    for property_key, property_value in properties.items():
        ...
        if property_value.get('properties'):
            ...
            model_data[property_key] = []
            for index, item in enumerate(data.get(property_key, [])):
